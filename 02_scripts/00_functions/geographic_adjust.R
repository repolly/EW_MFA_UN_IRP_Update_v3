...
# Function
geographic_adjust <- function(datainput){
  ...
  #  i <- former_list[3]
    ...
    for (j in firstyear:lastyear) {
     ...
     # j <- 1991
      ...
      for (k in unique(datainput$commodity)) {
        
     # k <- unique(datainput$commodity)
     #   k <- unique(datainput$commodity)[grep("natural gas",k)]
      ...
      if (sum(datainput[datainput$alphanumiso %in% countries & datainput$year == j & datainput$commodity == k,]$value, na.rm = TRUE) > 0  &
         (is.na(datainput[datainput$alphanumiso == i & datainput$year == j & datainput$commodity == k,]$value) ||  
         (!(i %in% unique(datainput[datainput$year == j & datainput$commodity == k,]$alphanumiso)))            
         )
         )
        {
        ...
        agg_values <- datainput %>%
          filter(alphanumiso %in% countries & year == j & commodity == k) %>%
          group_by(year, commodity, unit) %>% 
          summarise(new_value = sum(value, na.rm = TRUE)) %>%
          ungroup() %>% 
          mutate("alphanumiso" = i, "commodity" = k)
          
        
        
        # Join the aggregated values to the data table and include the relevant values
        datainput <- datainput %>%
          full_join(., agg_values, by = c("commodity", "year", "alphanumiso")) %>% 
        ...
        # datainput2 <- datainput
         #datainput %>% filter(year %in% 2002:2005, commodity == "Production of platinum group metals, mine", alphanumiso == "SRB688")
         #datainput %>% filter(year %in% 2002:2005, commodity == "Production of platinum group metals, mine", alphanumiso == "SCG891")
